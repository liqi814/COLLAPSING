...
readVcf <- function(vcf_file) {
    ...
    vf <- vf %>% mutate(`Variant Type` = case_when(VARIANT_CLASS == "SNV" ~ "snv",
        ...
        mutate(LOO_AF = (as.numeric(AC)-1)/(as.numeric(AN)-2)) %>% 
...
filterVcf <- function(vcf_data, effect, effect_impact, LOFTEE, polyphen, revel, 
                    primateai, exac_vqslod_snv, exac_vqslod_indel, gnomAD_rf_snv,
    ...
    vcf_filt <- vcf_filt %>% 
        ...
        filter(is.na(`LoF`) | `LoF` == "" | LoF %in% LOFTEE) %>%
        ...
        mutate(MultiScore = 0, NaScore = 0) %>%
        mutate(MultiScore = case_when(`Polyphen_char` %in% polyphen ~ MultiScore + 1, TRUE ~ MultiScore),
               NaScore = case_when(`Polyphen_char` == "unknown" ~ NaScore + 1, TRUE ~ NaScore),
               ) %>%
        mutate(MultiScore = case_when(REVEL_score >= revel ~ MultiScore + 1, TRUE ~ MultiScore),
               NaScore = case_when(is.na(REVEL_score) ~ NaScore + 1, TRUE ~ NaScore)) %>%
        mutate(MultiScore = case_when(PrimateAI_score >= primateai ~ MultiScore + 1, TRUE ~ MultiScore),
               NaScore = case_when(is.na(PrimateAI_score) ~ NaScore + 1, TRUE ~ NaScore)) %>%
        mutate(ToConsider = case_when(str_detect(Consequence, "missense_variant") ~ TRUE, TRUE ~ FALSE)) %>%
        mutate(MultiScore = case_when(ToConsider ~ MultiScore, TRUE ~ 3)) %>%
        filter(!ToConsider | MultiScore >= 2 | (MultiScore >= 1 & NaScore >=2)) %>% 
        mutate(IMPACT = factor(IMPACT, levels = c("HIGH", "MODERATE", "LOW", "MODIFIER"))) %>%
        group_by(`VariantID`) %>% 
        arrange(IMPACT, desc(MultiScore)) %>%
    ...
    variants <- variants %>% 
        ...
        mutate(gt_score = case_when(gt_GT %in% c("0/1", "1/0", "0|1", "1|0") & (gt_allele1 == ALT | gt_allele2 == ALT) ~ 1,
                                    gt_GT %in% c("1/1", "1|1") & (gt_allele1 == ALT & gt_allele2 == ALT) ~ 2,
                                    TRUE ~ 0))
...
getCollapseData <- function(model, cluster, chr) {
    ...
    if (model == "RareEnsemble") {
        ...
        revel <- 0
        primateai <- 0
        exac_vqslod_snv = 5000
        exac_vqslod_indel = 5000
        gnomAD_rf_snv = 0.01
        gnomAD_rf_indel = 0.02
        pop <- c("afr", "amr", "asj", "eas", "sas", "fin", "nfe")
        ext_af = 0
        ...
        stop("ERROR: This script is for dominiate models. Please enter model RareEnsemble, PTV or Synonymous!")
    }
    
    vcf_data <- readVcf(vcf_file)
    variants <- filterVcf(vcf_data, effect, effect_impact, LOFTEE, polyphen, revel, 
                          primateai, exac_vqslod_snv, exac_vqslod_indel, gnomAD_rf_snv,
                          gnomAD_rf_indel, pop, ext_af, loo_af)
...
FUNCTIONAL_EFFECTS <- c("exon_loss_variant", "frameshift_variant", "rare_amino_acid_variant", "stop_gained", "start_lost", "stop_lost", 
                        "splice_acceptor_variant", "splice_donor_variant", "gene_fusion", "bidirectional_gene_fusion", 
                        "3_prime_UTR_truncation+exon_loss_variant", "5_prime_UTR_truncation+exon_loss_variant", 
                        "coding_sequence_variant", "disruptive_inframe_deletion", "disruptive_inframe_insertion", 
...
FUNCTIONAL_EFFECTS_STRICT <- c("exon_loss_variant", "frameshift_variant", "rare_amino_acid_variant", "stop_gained", 
                               "start_lost", "stop_lost", "splice_acceptor_variant", "splice_donor_variant", "gene_fusion", 
                               "bidirectional_gene_fusion", "3_prime_UTR_truncation+exon_loss_variant", "5_prime_UTR_truncation+exon_loss_variant", 
                               "coding_sequence_variant", "disruptive_inframe_deletion", "disruptive_inframe_insertion", 
                               "conservative_inframe_deletion", "conservative_inframe_insertion", 
                               "missense_variant+splice_region_variant", "missense_variant")

MISSENSE_ONLY <- c("missense_variant+splice_region_variant", "missense_variant")

LOF_EFFECTS <- c("exon_loss_variant", "frameshift_variant", "rare_amino_acid_variant", "stop_gained", "stop_lost", "start_lost", "gene_fusion", "bidirectional_gene_fusion", "splice_acceptor_variant", "splice_donor_variant")
