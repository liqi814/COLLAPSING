...
readMatrix <- function(model, dir, resolution, pc_path, cluster) {
    sample.file <- paste0(pc_path, "/flashPCA_lclustering_res_", resolution, "_cluster_", cluster, "_sample.txt")
    matrix.file <- list.files(paste0("CollapsingAll/LClust_res_", resolution, "_cluster_", cluster, "_", dir, "FlashColl_07/"), "_matrix.csv.gz$", full.names = TRUE, recursive = TRUE)
    ...
    #data <- read.csv(matrix.file, delim = "\t", row.names=1)
...
doPermutations <- function(permut_path, matrix.data, perm) {
    ...
    saveRDS(cmhs.tidy, paste0(permut_path, '/', model, "_CMH_permut_perm_", perm, ".RDS"))
}

getCMHresP <- function(model, dir, pc_path, permut_path, IGMnclust, dbGAPnclust, IGM_resolution, dbGAP_resolution, nperm = c(1), cores = 1) {
    
    logfile <- paste0(permut_path, "/../Log/messagesperm_", dir, model, "_", nperm[1], "_", Sys.Date(), ".log")
    con <- file(logfile, "w")
    sink(con, append = TRUE, type = "message")
    print(model)
    
    matrix.data.IGM <- lapply(IGMnclust, function(x) readMatrix(model, dir, IGM_resolution, pc_path, x))
    matrix.data.dbGAP <- lapply(dbGAPnclust, function(x) readMatrix(model, dir, dbGAP_resolution, pc_path, x))
    matrix.data <- c(matrix.data.IGM, matrix.data.dbGAP)
    names(matrix.data) <- as.character(c(paste("IGM" ,IGMnclust,sep="."), paste("dbGAP",dbGAPnclust,sep=".")))
    
    mclapply(nperm, function(x) doPermutations(permut_path, matrix.data, x), mc.cores = cores)
    sink(type="message")
    close(con)
}
