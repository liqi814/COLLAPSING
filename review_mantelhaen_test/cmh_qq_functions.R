...
readPermTable <- function(dir, model, perm) {
    ...
    perm_file <- paste0(permut_path, model, "_CMH_permut_perm_", perm, ".RDS")

     if(!file.exists(perm_file)) {
        if(!str_detect(model, "recessive|dominant")) {
            model <- paste0("dominant", model)
            perm_file <- paste0(permut_path, dir, model, "_CMH_permut_perm_", perm, ".RDS")
        }
        if(!file.exists(perm_file)) {
            perm_file <- paste0(permut_path, dir, model, "_CMH_perm_", perm, ".RDS")
            if(!file.exists(perm_file)) {
            perm_file <- paste0(permut_path, dir, model, "_CMH_perm_", perm, ".RDS")            
            }
        }
...
plotQQ <- function(dir, model, cmh, nperm) {
    ...
    save(list=ls(), file=paste0(qq_path, model, "_qq_plot.RData"))
    ...
    ggsave(filename = paste0(qq_path, dir, model, "_CMH_qq_exact.pdf"), 
